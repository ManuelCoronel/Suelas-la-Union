<%-- 
    Document   : compras
    Created on : 13-dic-2020, 16:12:35
    Author     : ..
--%>
<%@page import="DTO.Estado"%>
<%@page import="DTO.PedidoDeVenta"%>
...
<html lang="en">
    ...
    <body>
        ...
        <main id="main">
            ...
            <section id="compras" class="portfolio section-bg">
                <div class="container" data-aos="fade-up" data-aos-delay="100">
                    ...
                    <div class="row">
                        <div class="col-12">
                            <%
                                if (user.getPedidoDeVentaList() != null) {
                            %>
                                    ...
                                    <div class="row offset-9">
                                        <div class="col col-6 mt-4">
                                            <select name="opcionesTipoDeEstados" class="form-control">
                                                ...
                                                <%
                                                    List<Estado> listEstado = (List<Estado>) request.getSession().getAttribute("tiposDeEstados");
                                                    for (Estado tipoEstado : listEstado) {
                                                %>
                            ...
                            <div class="table-responsive">
                                <table class="table table-hover">
                                    ...
                                    <tbody>
                                        <%
                                            if (user.getPedidoDeVentaList() != null) {
                                                List<PedidoDeVenta> pedidos = user.getPedidoDeVentaList();
                                                List<PedidoDeVenta> pedidosFiltros = (List<PedidoDeVenta>) request.getSession().getAttribute("pedidosBuscadosVenta");
                                                if (pedidosFiltros != null) {
                                                    pedidos = pedidosFiltros;
                                                }
                                                int indice = 0;
                                                for (PedidoDeVenta dto : pedidos) {
                                        %>
                                        <tr class="text-center">
                                            <td><%= ++indice%></td>
                                            <td><%=dto.getId()%></td>                                        
                                            <td><%=dto.getFechaOrden().getDate()+"/"+dto.getFechaOrden().getMonth()+"/"+(dto.getFechaOrden().getYear()+1900)%></td>               
                                            <td><%=(dto.getFechaEntregaFactura() != null) ? dto.getFechaEntregaFactura().getDate()+"/"+dto.getFechaEntregaFactura().getMonth()+"/"+(dto.getFechaEntregaFactura().getYear()+1900) : "-"%></td>
                                            <td><%=(dto.getFechaPlazo() != null) ? dto.getFechaPlazo().getDate()+"/"+dto.getFechaPlazo().getMonth()+"/"+(dto.getFechaPlazo().getYear()+1900) : "-"%></td> 
                                            <td><%=dto.getTipoEntregaId().getDescripcion()%></td>
                                            <td><%=dto.getPersonalId().getNombre() + " " + dto.getPersonalId().getApellido()%></td>
                                            <td><%=dto.getTotal()%></td>
                                            <td><%=dto.getEstadoId().getDescripcion()%></td>
                                            <td><a class="btn btn-warning" href="../../ControladorDetalles?accion=detalles&id=<%= dto.getId()%>">Detalles</a></td>      
                                        </tr>
                                        <%} 
                                            }%>
                                    </tbody>
